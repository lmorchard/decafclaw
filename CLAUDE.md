...
- `src/decafclaw/skills/background/` — Background process management skill (start, status, stop, list long-running processes). Bundled, auto-activates without confirmation.
- `src/decafclaw/skills/mcp/` — MCP admin skill (status, resources, prompts, restart). Bundled, auto-activates without confirmation.
...
- `src/decafclaw/preempt_search.py` — Keyword-match library for pre-emptive tool promotion: tokenize, match_tools, extract_last_assistant_text
