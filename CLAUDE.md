...
- **Panel-scoped `element[attr]` selectors capture nested components' internals.** `.panel input[type="text"]` (0,2,0) beats a shared component's own `.dc-chip-input` (0,1,0). Reach for a child combinator (`.panel label > input[type="text"]`) so the rule matches what it reads like.
