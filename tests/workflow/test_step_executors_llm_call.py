...
@pytest.mark.asyncio
async def test_execute_user_input_no_config_raises(ctx, workflow_state):
    """execute() with user_input step and no config raises RuntimeError.

    All step kinds are now implemented (Phase 5). user_input with no
    'input: text' and no choices is invalid configuration — RuntimeError.
    """
    step = StepDef(
        id="input_step",
        kind=StepKind.USER_INPUT,
        config={},  # no 'input' key, no choices
    )
