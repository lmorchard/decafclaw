...
- **SCHEDULE.md body duplicated SKILL.md body.** I noticed the tension during planning ("the simplest path is 'preserve current behavior'") and chose the duplication path, deferring the cleaner refactor. Les caught it post-retro and pushed back. The right design — and what shipped — is a thin trigger in SCHEDULE.md (e.g., "Time for the daily dream pass. Follow the dream skill instructions to completion.") plus `required-skills: [<self>, ...]` so the SKILL.md body is the single source of truth, activated into context for the scheduled run. Lesson: when a "preserve current behavior" shortcut requires literal duplication, that's a smell. Either lift the duplication into a thin reference now or call it out as a follow-up explicitly, but don't silently ship it.
