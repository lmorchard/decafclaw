...
- [ ] `make test` passes
- [ ] `pytest tests/workflow/ -v` — all phase-1 unit tests green
- [ ] `make eval-workflows` (or equivalent invocation) runs the hello case end-to-end against a real LLM and asserts the workflow reaches `RunStatus.DONE`
...
- [ ] `pytest tests/workflow/test_step_executors_tool_call.py -v` — new tests green
- [ ] `make eval-workflows` — hello case now includes tool_call step in assertions
...
**Verification — automated:**
- [ ] `make lint` / `make typecheck` / `make test` pass
- [ ] `pytest tests/workflow/test_step_executors_subagent.py -v` — new tests green (mocked subagent dispatch)
- [ ] `make eval-workflows` — partial research_brief case completes (gather subagent runs, read_sources fires, terminal reached)
...
**Verification — automated:**
- [ ] `make lint` / `make typecheck` / `make test` pass
- [ ] `pytest tests/workflow/test_step_executors_route.py tests/workflow/test_step_executors_python.py -v` — new tests green
- [ ] `make eval-workflows` — full research_brief eval case completes (gather → ... → publish), Flash succeeds first-try (matching spike's known-good run)
...
- [ ] `make eval-workflows` — interview case reaches `PAUSED_USER_INPUT` and asserts pending prompt content (full multi-turn interview eval deferred to a later session if the eval framework gets synthetic-response injection)
