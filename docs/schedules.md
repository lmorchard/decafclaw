...
| `pre_script` | string | no | — | Python script run **before** the turn; its stdout is injected into the prompt. Relative to `data/{agent_id}/` (trusted schedules only, with the agent-writable `workspace/` subdirectory explicitly excluded). Not run at all outside admin or bundled tier — see [Permissions are tier-dependent](#permissions-are-tier-dependent). See [Pre-agent scripts](#pre-agent-scripts). |
...
- **Path is sandboxed.** Resolved against `data/{agent_id}/` for trusted schedules, with the agent-writable `workspace/` subdirectory explicitly excluded and blocked to prevent agent-authored script shadowing/hijacking. Containment checked after resolution — `../` escapes are rejected rather than followed.
