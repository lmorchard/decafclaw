...
7.  **Handle Someday Items**: Use `md_show` on yesterday's note to find unchecked `- [ ]` items in the `someday` section. Call `md_move_lines` to migrate these items to the `pages/someday.md` file.
...
-   **`md_move_lines`**: **WARNING:** If the `to_file` path does not exist, this tool will **create it automatically**. If you use this tool without first ensuring today's note was created from its template, you will end up with a blank file containing only the moved tasks, completely bypassing the intended template structure. This is why the **Check-then-Create** procedure is mandatory.
