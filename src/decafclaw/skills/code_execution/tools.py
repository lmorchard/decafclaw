...
TOOL_DEFINITIONS = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "`.error`. Prefer `.data` for filtering/sorting/aggregating "
                "(e.g. `dc.workspace_list(path='conversations').data["
                "'entries']` is a list of `{name, is_dir, size}` dicts; "
                "`dc.vault_search(query='x').data['results']` is a list "
                "of result rows; `dc.tabstack_extract_json(url=u, "
                "json_schema=s).data['result']` is the parsed object — "
                "no json.loads needed). Body/content payloads (vault page "
                "bodies, workspace file content) stay in `.text` only. "
                "`print(...)` what you want returned to the conversation."
            ),
            "parameters": {
                "type": "object",
                "properties": {
