...
- **`vault_list` is the enumeration tool, not `vault_search`.** A `vault_search` call that narrows on nothing — no `query`, `tags`, `folder` or `days` — is refused rather than run, because it would return every page in the vault instead of searching it.
