...
async def _execute_user_input(ctx, step: StepDef,
    ...
    """user_input step — requires Phase 3 wiring via ctx.request_confirmation.

    Phase 3 rewrites this to await a ConfirmationRequest directly so the
    engine never returns to the agent loop while waiting for user input.
    Until then, any workflow containing a user_input step will raise here.
    """
    raise NotImplementedError(
        "user_input requires Phase 3 wiring (ctx.request_confirmation). "
        "This step kind is not yet available in Phase 1."
    )
