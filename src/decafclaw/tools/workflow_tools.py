...
def _transform_response(response_data: dict, mode: str) -> dict:
    ...
    mode. Matches the shape expected by ``engine.resume_user_input``.
    """
    if mode == "text":
...
def _make_on_response(ctx: Any, state: WorkflowState, mode: str) -> Any:
    ...
    async def on_response(response_data: dict) -> str:
        ...
        while True:
            ...
            try:
                await engine.resume_user_input(ctx, state, user_input)
                log.info(
...
WORKFLOW_TOOL_DEFINITIONS = [
    {
        ...
        "function": {
            "name": "workflow_start",
            "description": (
                "Start a workflow by name. The workflow engine runs the "
                "step graph to completion or until a suspension point "
                "(user input required). Returns the final status and any "
                "structured outputs produced by the workflow steps."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
                        "type": "string",
                        "description": "The registered workflow name to run.",
                    },
                },
                "required": ["name"],
            },
